  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/config') {
      ...
      const inviteUrl = clientId
        ...
        : '';
      return json({ clientId, inviteUrl, installation: 'guild', botPermissions: BOT_PERMISSIONS }, 200, cors);
    }
    ...
    const structure = url.pathname.match(/^\/api\/guilds\/([^/]+)\/structure$/);
    if (structure && request.method === 'GET') {
      const guildId = structure[1];
      const guildResponse = await discord(env, `/guilds/${guildId}`);
      if (!guildResponse.ok) return discordError(guildResponse, cors);
      const channelResponse = await discord(env, `/guilds/${guildId}/channels`);
      if (!channelResponse.ok) return discordError(channelResponse, cors);
      const guild = await guildResponse.json();
      const channels = await channelResponse.json();
      return json({
        guild: { id: guild.id, name: guild.name, icon: guild.icon ? `https://cdn.discordapp.com/icons/${guild.id}/${guild.icon}.png?size=128` : null },
        channels: channels
          .filter(channel => [0, 2, 5, 13, 15].includes(channel.type))
          .sort((a, b) => (a.position ?? 0) - (b.position ?? 0))
          .map(channel => ({ id: channel.id, name: channel.name, type: channel.type, position: channel.position, parentId: channel.parent_id, nsfw: !!channel.nsfw }))
      }, 200, cors);
    }
    ...
    if (channels && request.method === 'GET') {
      ...
      return json(data.filter(channel => channel.type === 0).sort((a, b) => (a.position ?? 0) - (b.position ?? 0)).map(channel => ({ id: channel.id, name: channel.name, position: channel.position, categoryId: channel.parent_id })), 200, cors);
    }
    ...
    if (messages && request.method === 'POST') {
      if (!env.WRITE_API_KEY || request.headers.get('x-omnibot-key') !== env.WRITE_API_KEY) {
        return json({ error: 'Web posting is disabled.' }, 403, cors);
      }
    ...
    return new Response('OmniBot Worker online', { headers: { ...cors, 'content-type': 'text/plain; charset=utf-8' } });
  }
};
...
export class BotBridge {
  constructor(ctx, env) { this.ctx = ctx; this.env = env; this.gateway = null; this.clients = new Map(); this.guilds = new Map(); this.seq = null; this.heartbeat = null; this.reconnectTimer = null; this.starting = false; this.sessionId = null; this.resumeUrl = null; this.lastGatewayError = null; }
  async fetch(request) {
    const url = new URL(request.url);
    if (url.pathname === '/health') { this.ensureGateway(); return json({ ok: true, gatewayConnected: !!this.gateway, gatewayStarting: this.starting, tokenConfigured: !!this.env.DISCORD_TOKEN, clientIdConfigured: !!this.env.DISCORD_CLIENT_ID, browserClients: this.clients.size, guilds: [...this.guilds.values()], guildCount: this.guilds.size, lastGatewayError: this.lastGatewayError }); }
    if (url.pathname === '/guilds') { this.ensureGateway(); return json([...this.guilds.values()]); }
    const known = url.pathname.match(/^\/guild\/([^/]+)\/known$/);
    if (known) { this.ensureGateway(); return this.guilds.has(known[1]) ? json({ ok: true }) : json({ error: 'The bot is not connected to this server.' }, 404); }
    ...
    safe(socket, { type: 'guilds', data: [...this.guilds.values()] });
    this.ensureGateway();
    return new Response(null, { status: 101, webSocket: client });
  }
  browserMessage(id, raw) { try { const message = JSON.parse(raw), client = this.clients.get(id); if (!client) return; if (message.type === 'subscribe') { client.guildId = String(message.guildId || ''); client.channelId = String(message.channelId || ''); safe(client.socket, { type: 'subscribed', guildId: client.guildId, channelId: client.channelId }); } } catch {} }
  ...
  async connectGateway() {
    try {
      ...
      if (!discovery.ok) throw new Error(`Gateway discovery failed (${discovery.status}): ${await discovery.text()}`);
      const info = await discovery.json();
      this.resumeUrl = info.url;
      const wsResponse = await fetch(`${info.url}?v=${GATEWAY_VERSION}&encoding=json`, { headers: { Upgrade: 'websocket' } });
      if (!wsResponse.webSocket) throw new Error('Cloudflare did not create the outbound Gateway WebSocket');
      this.gateway = wsResponse.webSocket;
      this.gateway.accept();
      this.gateway.addEventListener('message', event => this.gatewayMessage(event.data));
      this.gateway.addEventListener('close', event => this.gatewayClosed(event));
      this.gateway.addEventListener('error', () => this.gatewayClosed());
    } catch (error) { this.lastGatewayError = error?.message || String(error); console.error('[OmniBot Gateway]', error); this.gateway = null; this.scheduleReconnect(5000); }
    finally { this.starting = false; }
  }
  gatewayMessage(raw) {
    let payload; try { payload = JSON.parse(raw); } catch { return; }
    if (payload.s !== null && payload.s !== undefined) this.seq = payload.s;
    if (payload.op === 10) { this.startHeartbeat(payload.d.heartbeat_interval); this.send(2, { token: this.env.DISCORD_TOKEN, intents: INTENTS, properties: { os: 'linux', browser: 'omnibot', device: 'omnibot' } }); return; }
  ...
  dispatch(type, data) {
    if (type === 'READY') { this.sessionId = data.session_id; this.resumeUrl = data.resume_gateway_url || this.resumeUrl; this.guilds.clear(); for (const guild of data.guilds || []) this.guilds.set(guild.id, { id: guild.id, name: guild.name, icon: guild.icon ? `https://cdn.discordapp.com/icons/${guild.id}/${guild.icon}.png?size=128` : null }); const payload = [...this.guilds.values()]; this.broadcast({ type: 'status', gatewayConnected: true, guildCount: payload.length }); this.broadcast({ type: 'guilds', data: payload }); return; }
    if (type === 'GUILD_CREATE' || type === 'GUILD_UPDATE') { this.guilds.set(data.id, { id: data.id, name: data.name, icon: data.icon ? `https://cdn.discordapp.com/icons/${data.id}/${data.icon}.png?size=128` : null }); this.broadcast({ type: 'guilds', data: [...this.guilds.values()] }); return; }
  ...
  gatewayClosed(event) { this.gateway = null; if (this.heartbeat) clearInterval(this.heartbeat); this.heartbeat = null; this.lastGatewayError = event ? `Gateway closed (${event.code || 'unknown'}): ${event.reason || ''}`.trim() : this.lastGatewayError; this.broadcast({ type: 'status', gatewayConnected: false, guildCount: this.guilds.size }); this.scheduleReconnect(5000); }
